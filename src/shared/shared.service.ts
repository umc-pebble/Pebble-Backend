...
export const sharedService = {
  ...
  // 탈퇴해도 본인이 그 카테고리에 만든 마일스톤·태스크는 완료 여부와 관계없이 그대로 유지된다
  // 완료 상태는 유지하고 본인이 완료 처리한 항목의 처리자만 null로 변경한다.
  async leaveSharedCategory(userId: number, categoryId: number) {
    ...
    await sharedRepository.deleteMembershipAndClearCompletionActor(categoryId, userId);
  },

  // 오너가 특정 멤버를 강퇴한다. 강퇴 알림은 발송하지 않는다 (PLB-045).
  // 모든 콘텐츠와 완료 상태는 유지하고, 강퇴 대상이 완료 처리한 항목의 처리자만 null로 변경한다.
  async removeMember(ownerId: number, categoryId: number, targetUserId: number) {
    ...
    await sharedRepository.deleteMembershipAndClearCompletionActor(categoryId, targetUserId);
  },
