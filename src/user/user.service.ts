...
import { Prisma, DateType } from '@prisma/client';
...
export const userService = {
  ...
  // 프로필 내 수 놓은 조약돌 개수 조회 함수.
  // 마이페이지 내에 위치하지만, 내 기본 프로필 정보와는 다르게 테스크 완료 처리 변경 여부 등에 따라
  // 자주 바뀌는 데이터이기 때문에 따로 만들어진 api에 적용되는 함수.
  // 지금까지(기간 제한 없이) 완료 처리된 태스크 전체 개수를 조약돌 1개당 1개씩 센다.
  // MULTIPLE 태스크는 상위 Task.isCompleted를 쓰지 않고 회차별 TaskDate.isCompleted로 완료를 추적하므로
  // (task.service.ts의 toggleTaskComplete 참고), 상위 태스크 완료 수와 MULTIPLE의 회차별 완료 수를 따로 센다.
  async getPebble(userId: number) {
    await getUserOrThrow(userId);

    const [completedTaskCount, completedTaskDateCount] = await Promise.all([
      prisma.task.count({
        where: { userId, isCompleted: true, dateType: { not: DateType.MULTIPLE } },
      }),
      prisma.taskDate.count({
        where: { isCompleted: true, task: { userId, dateType: DateType.MULTIPLE } },
      }),
    ]);

    return { pebble: completedTaskCount + completedTaskDateCount };
  },
