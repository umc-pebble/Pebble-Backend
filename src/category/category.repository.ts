...
export const categoryRepository = {
  ...
  //  - 아래 FOR UPDATE는 갭 락 교착(P2034)을 낼 수 있으므로, 호출부가 회원탈퇴 트랜잭션 전체를
  //    재시도 루프로 감싸야 한다(create가 자기 트랜잭션에 두른 것과 같은 판정·횟수). 이 함수는
  //    호출부 트랜잭션에 합류하는 쪽이라 여기서 재시도하면 이미 깨진 트랜잭션 위에서 다시 도는 꼴이 된다.
  //
