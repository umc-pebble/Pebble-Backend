 ...
 *
 *       공유 카테고리에는 두 가지 조건이 더 붙습니다. 이 화면이 "대상 유저의 일정을 보는" 용도라,
 *       대상 유저와 무관한 정보가 딸려 나가지 않게 하기 위함입니다.
 *       (1) **오너도 요청자와 친구여야** 합니다 — isPublic은 오너가 자기 팔로워에게 건 공개 설정이라,
 *       대상 유저가 멤버라는 이유만으로 열어주면 오너가 공개한 적 없는 상대에게 내용이 노출됩니다.
 *       (2) **대상 유저가 그 카테고리에 마일스톤이나 태스크를 하나라도 작성했어야** 합니다 —
 *       아무것도 쓰지 않은 카테고리까지 내려가면 일정은 없이 남의 카테고리 이름과 색만 노출됩니다.
 *       이 판정은 조회 월과 무관하게 전체 기간 기준이므로, 목록에 있어도 특정 월에는 비어 보일 수 있습니다.
 *       (대상 유저가 소유한 카테고리는 비어 있어도 그대로 노출됩니다 — 기존 동작 유지)
 *
